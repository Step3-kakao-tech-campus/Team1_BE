...
class RecommendedWeeklyScheduleRepositoryTest extends BaseTest {
	...
	public RecommendedWeeklyScheduleRepositoryTest(UserRepository userRepository, GroupRepository groupRepository,
		...
		RecommendedWeeklyScheduleRepository recommendedWeeklyScheduleRepository,
		RecommendedWorktimeApplyRepository recommendedWorktimeApplyRepository, EntityManager em1) {
	...
	void test1() {
		...
		Group group = userRepository.findGroupByUser(1L).orElse(null);
		...
		List<Worktime> weeklyWorktimes = worktimeRepository.findByStartDateAndScheduleId(selectedDate,
			schedule.getId());
		...
		int selected = 1;
		List<RecommendedWeeklySchedule> recommendedSchedule = recommendedWeeklyScheduleRepository.findByUser(
			user.getId());
		...
		recommendedSchedule.forEach(
			x -> recommendedWorktimeApplyRepository.deleteAll(x.getRecommendedWorktimeApplies()));
